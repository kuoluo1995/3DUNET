...
def create_data_yaml(path):
    ...
    for t1_file in path.iterdir():
        ...
        if t1_image.shape == seg_image.shape:
            paired_data.append({'t2': str(t1_file), 'seg': str(seg_file)})
    shuffle(paired_data)
    yaml_utils.write(str(output_path / 't22seg_train.yaml'), paired_data[:8 * len(paired_data) // 10])
    yaml_utils.write(str(output_path / 't22seg_test.yaml'), paired_data[8 * len(paired_data) // 10:])
...
def data_loader():
    ...
    train_generator = data_generator(train_list, 6)
    test_generator = data_generator(test_list, 12)
...
if __name__ == '__main__':
    ...
    _model = unet_3d(input_shape=(1, 64, 64, 32))  # channels,shape
    # Path('_').mkdir(parents=True, exist_ok=True)
    _model.fit_generator(generator=train_generator, steps_per_epoch=train_steps, epochs=200,
